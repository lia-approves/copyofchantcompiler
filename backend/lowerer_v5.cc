...
void VarCountVisitor::VisitGreaterThanEqualToExpr(
...
void VarCountVisitor::ScanningParams(bool scanningParams) {
  readingParams_ = scanningParams;
}
...
void VarCountVisitor::AddVariable(string variable) {
  bool foundinParams = std::find(
    paramVariables_.begin(), paramVariables_.end(),
...
void IrGenVisitor::VisitVariableExpr(const VariableExpr& exp) {
  ...
  if (requestAddressFromNextNode == true) returnAddress = true;
  std::shared_ptr <StatementNode>newhead;
  if (!readingParams_) {
    if (returnAddress == true) {
      // push value or address depending on request from parent node
...
void IrGenVisitor::VisitDereference(const Dereference& exp) {
  bool returnAddress = false;
  if (requestAddressFromNextNode == true) returnAddress = true;
  // we need address from lsh, request adress
  requestAddressFromNextNode = true;
  exp.lhs().Visit(this);
  requestAddressFromNextNode = false;  // need just value from rhs
  exp.rhs().Visit(this);
  // index is at top of stack(lhs), adress is at 2nd to top(lhs)
  std::unique_ptr<Operand> index = std::move(ir_stack_.back());
  ir_stack_.pop_back();
  // ex: x[4] x is address, 4 is index
...
void IrGenVisitor::VisitProgram(const Program& program) {
  // we visit func def firs they go at top of assmbly
  ...
  std::shared_ptr<StatementNode> newhead = std::make_shared< StatementNode>(
    make_unique<Label>(labelNum_++),
    nullptr,
    // this is so we know how many local vars to alocate on the stack
    make_unique<Operator>(Operator::kProgramStart),
    nullptr,
...
void IrGenVisitor::VisitFunctionCall(const FunctionCall& call) {
  ...
  for (auto& arg : call.arguments()) {
     // visit args place on stack need to fix this its backwards
    arg->Visit(this);
    std::shared_ptr<StatementNode> newhead = std::make_shared< StatementNode>(
      make_unique<Label>(labelNum_++),
      make_unique<Register>(register_number_ - 1),
...
void IrGenVisitor::VisitFunctionDef(const FunctionDef& def) {
  // we count number of params and local vars
  VarCountVisitor varsCounter;
  // count params first we dont want to redeclare var if its in params already
