...
class AsmProgram {
 ...
 private:
  ...
  // used by switch so we dont redeclare and dont have to create new scope
  int offSet_ = 0;
};
void AsmProgram::IrToAsm(IrGenVisitor* ir) {
  ...
  while (head != nullptr) {
    asm_sstring_
      << endl << "statementnumber_" << head->GetLabel().GetValue()
      << ":" << endl;
    GenerateASM(head);
    head = head->GetNext();
  }
...
void AsmProgram::GenerateASM(std::shared_ptr<StatementNode> node) {
  ...
  case Operator::kPushValueOfInteger:
    asm_sstring_
     // only integer visitor uses this
      << "push $" << node->GetOp2().GetValue() << endl;
    break;
  case Operator::kPushValueOfVariable:
    ...
    // if we want the value at address we put parenthesis around
    asm_sstring_ << "push (%rcx)" << endl;
    break;
  ...
  //  index is at the top of stack, address is at second to last,
  // which has location of pointer as value
    asm_sstring_
      ...
      //  mov pointer address into rax, if we wrote %rcx without parent,
      // we would be moving the address which contains the value of the
      // pointer, not the pointer
      << "mov (%rcx), %rax" << endl
      // add 8*index to address of rax(base array pointer)
      << "add %rbx,%rax" << endl
      // push address what is at %rax(location of array + index)
      << "push %rax" << endl;
    break;
  case Operator::kPushValueOfDereference:
    asm_sstring_
      ...
      // add 8*index to address of rax(base array pointer)
      << "add %rbx,%rax" << endl
      // push value of what is at %rax(location of array + index)
      << "push (%rax)" << endl;
    break;
  case Operator::kAssignmentFromNewTuple:
  //  we store adress of heap at variable directly
    asm_sstring_
      ...
      // start of heap saved in rsi, this is our pointer
      << "mov %rax, %rsi" << endl
      //  prepare to move heap start by moving breakpoint we need to put
      // desired address in rdi
      << "mov %rax, %rdi" << endl
      << "pop %rax" << endl  //  size of desired tuple in rax
      //  allocate one more byte just in case some crazy mofo wants to
      // start arrays at 1
      << "inc %rax" << endl
      << "mov $8, %rbx" << endl  // size times 8
      << "imul %rax, %rbx" << endl  // multiply, result in rbx
      // move heap breakpoint by 8 * size of tuple, size was in rbx
      << "add %rbx, %rdi" << endl
      << "mov $12, %rax" << endl  //  prepare brk syscall number again
      << "syscall" << endl  //  new heap breakpoint has been moved
      //  place adress of reference variable into %rcx (lhs), it was on
      // the stack
      << "pop %rcx" << endl
      //  move heap pointer(the start) into the location of
      // %rcx(address of variable)
      << "mov %rsi, (%rcx)" << endl;
       //  at the end, the program break is moved so we dont need to have a
       // new base, every time we request the address of the program
       // break(start of heap), we get a fresh address
    break;
  case Operator::kAssignmentFromArithExp:
    asm_sstring_
    // we visit rhs second so value is a top put it in rbx
      << "pop %rbx" << endl
      // address is next on stack place it at rax
      << "pop %rax" << endl
      // move value rbx to location at address rax
      << "mov %rbx, (%rax)" << endl;
    break;
  ...
  case Operator::kCall:
    ...
    asm_sstring_
      ...
      //  move value to variable( call.lhs.name)
      << "pop " << offSet_ << "(%rbp)" << endl
      // should change this whole routine soon it works for now
      << "add $" << 8 * node->GetOp2().GetValue() << ", %rsp" << endl;
    break;
  case Operator::kProgramStart:
    ...
    asm_sstring_
      ...
      << "mov %rsp, %rbp" << endl
      // allocate local vars
      << "sub $" << 8 * node->GetOp2().GetValue() << ", %rsp" << endl;
    break;
  case Operator::kProgramEnd:
    asm_sstring_
      << "#### End of Statements ####" << endl
      << "pop %rax" << endl  // ae will be in rax
      // this is for gdb debugging dont remove
      << "GDB_ENDPROG_BREAKPOINT:" << endl
      << "mov $format, %rdi" << endl
      << "mov %rax, %rsi" << endl
      << "mov $0, %rax" << endl
...
int AsmProgram::GetOffSet(string variable) {
  int pos;
  int stackOffset;
  bool foundinParams = std::find(paramVariables_.begin(), paramVariables_.end(),
   (variable)) != paramVariables_.end();
  if (foundinParams) {
    pos = std::distance(paramVariables_.begin(), std::find
    (paramVariables_.begin(), paramVariables_.end(), variable));
    stackOffset = 1 * ((pos + 2) * 8);
  } else {
    bool foundinLocal = std::find(
      localVariables_.begin(), localVariables_.end(),
      (variable)) != localVariables_.end();
    if (!foundinLocal) { localVariables_.push_back(variable); }
    pos = std::distance(localVariables_.begin(), std::find(
      localVariables_.begin(), localVariables_.end(), variable));
