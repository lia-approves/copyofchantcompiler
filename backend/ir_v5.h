...
class StatementNode {
 public:
  ...
  void Print() {
    ...
    case Operator::kAdd:
      cout << GetTarget().GetName() << " = " << GetOp1().GetName() << " + "
      << GetOp2().GetName();
      break;
    ...
    case Operator::kLessThan:
      cout << "if (" << GetOp1().GetName() << " < " << GetOp2().GetName()
      << ") goto S" << GetTarget().GetValue() << ":";
      break;
    ...
    case Operator::kProgramEnd:
      ...
      break;
    case Operator::kCall:
      cout << "call " << GetTarget().GetName() << "," << GetOp2().GetValue()
      << "  --> " << GetOp1().GetName();  // op2 is num of args
      break;
    ...
    case Operator::kAssignmentFromArithExp:
      // a=5  a is target 2 is in op2, if theres is a single argument in the
      // Three adress code, we normally put it in the second op field
      cout << "*" << GetTarget().GetName() << " = " << GetOp2().GetName();
      break;
    case Operator::kAssignmentFromNewTuple:
      cout << "*" << GetTarget().GetName() << " = newTuple("
      << GetOp2().GetName() << ")";
      break;
    }
  }
