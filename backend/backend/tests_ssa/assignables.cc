...
TEST(Call, FunctionTwice) {
  ...
  AsmProgram testasm;
  testasm.SSAIRToAsm(ssatest.GetSSAIR());
  int dominates0 = ((ssatest.GetDominators()[0]).GetDominated())[0][0];
  int dominates11 = ((ssatest.GetDominators()[1]).GetDominated())[0][0];
  ...
  EXPECT_EQ("52", output);
  EXPECT_EQ(ssatest.GetDominators()[0].GetBlockNums()[0], dominates0);
  EXPECT_EQ(ssatest.GetDominators()[1].GetBlockNums()[0], dominates11);
}
