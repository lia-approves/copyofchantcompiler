...
TEST(AE, NestedTuplesRootDominates) {
  Statement::Block foo_statements;
  FunctionDef::Block function_defs;
  ...
  std::vector<int> dominatesans = {0, 5, 6, 9, 12, 13, 16, 17, 18, 19, 22, 24, 27, 28, 29, 34};
...
TEST(AE, NestedTuples24Dominates) {
  Statement::Block foo_statements;
  FunctionDef::Block function_defs;
  ...
  // see that basic block 24 dominates 24, 27, 28, 29, and 34
  std::vector<int> dominates =
    ssatest.GetDominators()[0].GetBlockNums();
  int index24 = std::distance(dominates.begin(), 
    std::find(dominates.begin(), dominates.end(), 24));
  std::vector<int> dominates24 = 
    ((ssatest.GetDominators()[0]).GetDominated())[index24];
  std::vector<int> dominatesans = {24, 27, 28, 29, 34};
  for (int i = 0; i < dominatesans.size(); i++) {
    auto it = std::find(dominates24.begin(), dominates24.end(), dominatesans[i]);
    EXPECT_EQ(*it, dominatesans[i]);
  }

}

TEST(AE, NestedTuplesDominanceFrontiers) {
  Statement::Block foo_statements;
  FunctionDef::Block function_defs;
  ...
  // see that the basic block with id 13 has dominant frontiers 16 and 17
  std::vector<int> dominates13 =
    ssatest.GetDominators()[0].GetBlockNums();
  int index13 = std::distance(dominates13.begin(), 
    std::find(dominates13.begin(), dominates13.end(), 13));
  std::vector<int> domfronts13 = 
    ((ssatest.GetDominators()[0]).GetDomFrontiers())[index13];
  std::vector<int> domfronts13ans = {16, 17};
  for (int i = 0; i < domfronts13ans.size(); i++) {
    auto it = std::find(domfronts13.begin(), domfronts13.end(), domfronts13ans[i]);
    EXPECT_EQ(*it, domfronts13ans[i]);
  }
