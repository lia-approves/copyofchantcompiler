...
TEST(v5, CanLoopDominates) {
  ...
  // see that basic block 5 dominates blocks 5, 8, 9, and 18
  std::vector<int> dominates =
    ssatest.GetDominators()[0].GetBlockNums();
  int index5 = std::distance(dominates.begin(), 
    std::find(dominates.begin(), dominates.end(), 5));
  std::vector<int> domfronts5 = 
    ((ssatest.GetDominators()[0]).GetDominated())[index5];
  std::vector<int> dominatesans = {5, 8, 9, 18};
  for (int i = 0; i < dominatesans.size(); i++) {
    auto it = std::find(domfronts5.begin(), domfronts5.end(), dominatesans[i]);
    EXPECT_EQ(*it, dominatesans[i]);
  }
}

TEST(v5, CanLoopDominanceFrontiers) {
  Statement::Block statements;
  FunctionDef::Block function_defs;
  Statement::Block loop_body;
  ...
  // see that the basic block with id 5 has frontier 5
  std::vector<int> dominates5 =
    ssatest.GetDominators()[0].GetBlockNums();
  int index5 = std::distance(dominates5.begin(), 
    std::find(dominates5.begin(), dominates5.end(), 5));
  std::vector<int> domfronts5 = 
    ((ssatest.GetDominators()[0]).GetDomFrontiers())[index5];
  std::vector<int> domfronts5ans = {5};
  for (int i = 0; i < domfronts5ans.size(); i++) {
    auto it = std::find(domfronts5.begin(), domfronts5.end(), domfronts5ans[i]);
    EXPECT_EQ(*it, domfronts5ans[i]);
  }

  // see that the basic block with id 9 has frontier 5
  std::vector<int> dominates9 =
    ssatest.GetDominators()[0].GetBlockNums();
  int index9 = std::distance(dominates9.begin(), 
    std::find(dominates9.begin(), dominates9.end(), 9));
  std::vector<int> domfronts9 = 
    ((ssatest.GetDominators()[0]).GetDomFrontiers())[index9];
  std::vector<int> domfronts9ans = {5};
  for (int i = 0; i < domfronts9ans.size(); i++) {
    auto it = std::find(domfronts5.begin(), domfronts5.end(), domfronts9ans[i]);
    EXPECT_EQ(*it, domfronts9ans[i]);
  }
